...
> **Compatibility:** tested against the **[BMAD-METHOD](https://github.com/bmad-code-org/BMAD-METHOD) v6 skill line**, currently up to **6.8.0** (and the **6.8.1-next.2** prerelease).
...
- **Code review starts on Opus** and alternates Opus/Sonnet across iterations. It **always runs at least two review passes** (so the alternate model always weighs in), then exits the loop once a pass found-and-fixed ≤3 non-deferred findings with no Critical/High remaining — otherwise it re-reviews, up to the iteration cap (default 3). The loop **always ends by asking whether to continue**, suggesting you run an external review (a human, another model/AI) on the changes while the pipeline is paused; on continue it summarizes anything you added before resuming. If the loop hit the cap still unconverged, the eventual PR opens as a **draft**.
...
| 7 | Code review (Opus-first, alternating models, 2–3 iters; always halts for your go-ahead) | `bmad-code-review` | always |
