...
| 7 | Code review — Opus-first, alternating models; runs ≥2 passes unless the first is perfectly clean, then exits once a pass found-and-fixed ≤3 non-deferred findings with no Critical/High remaining (cap 3); halts for your go-ahead (skippable on a clean convergence — see config) | `bmad-review-adversarial-general`, `bmad-review-edge-case-hunter` (`bmad-code-review`'s adversarial layers, run as parallel sub-agents + triage; on opencode the lenses run sequentially) | always |
...
- **Code review** — iteration cap, model alternation, and `skip_hitl_on_clean_convergence` (default off; skip the Phase 7 human halt when the review converged cleanly).
