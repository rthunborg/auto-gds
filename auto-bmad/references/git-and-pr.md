...
- **CI link:** if the repo has CI workflows (`.github/workflows/*.yml|*.yaml`), the push/PR will
  have triggered a run. Capture its URL for the report — query the run for the pushed head SHA:
  ...
  Store it in state (`ci_run_url`). **Do not wait for CI to finish** — just link the run (note its
  status, e.g. "queued/in_progress", in the report). Skip this entirely if there are no workflow
  files.
