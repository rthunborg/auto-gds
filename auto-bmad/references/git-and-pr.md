...
- **epic mode** (`/auto-bmad epic`) owns the same git set at **epic scope** — one `epic/{e}-{slug}`
  ...
  BMAD-status flip, and the epic-anchor finalize — plus the single E_review HITL halt. The re-review
  there is delegated like any other. Details: "Epic mode" below + `epic-pipeline.md`.
...
- **PR title/body:** title `feat(epic-{e}): <epic summary>`. Body: a one-paragraph epic summary; a
  **per-story rollup** (one line per landed story — status + one-line outcome + thin-review
  ...
  checklist aggregating open `[Review][Decision]` / deferred items across all stories + E_review's open
  findings + E8a gaps; the `🤖 Generated by auto-bmad` footer.
