...
the two writes can only duplicate an entry, never lose one). ``--expect-sha``
must equal the current ledger sha256 (take it from ``plan``); a mismatch — the
file changed since planning — or an unknown id exits 1 with NO writes. Prints::

    {"moved": int, "headings_created": int, "headings_removed": int,
...
def _insert_entries(segments, next_section, moved) -> int:
    """Insert moved entries into the archive's segment model. Returns the
    number of headings created (identical existing headings are reused)."""
    created = 0
...
def do_archive(ledger_path: str, archive_path: str, ids, expect_sha: str):
    ...
    if os.path.isfile(archive_path):
        ...
        a_next = 0
    created = _insert_entries(a_segments, a_next, moved)
